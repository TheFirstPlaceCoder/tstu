...
public record Equation(int a, int b, int c) {
    ...
    public static final int MAX_COEFFICIENT_ABS_VALUE = 10_000;
    ...
    public Equation {
        ...
        if (a == 0 && b == 0) {
            throw new MathToolException("это не уравнение, неизвестное отсутствует");
        }
    }
    ...
    public static int parseCoefficient(String value, String name) {
        if (value == null || value.isBlank()) {
            throw new MathToolException("коэффициент " + name + " не задан");
        }
        ...
        try {
            ...
            throw new MathToolException("коэффициент " + name + " не является целым числом");
        }
    ...
    private static void validateRange(int value, String name) {
        if (Math.abs((long) value) > MAX_COEFFICIENT_ABS_VALUE) {
            throw new MathToolException(
                    "коэффициент " + name + " находится вне допустимого диапазона [-"
                            + MAX_COEFFICIENT_ABS_VALUE + "; " + MAX_COEFFICIENT_ABS_VALUE + "]"
            );
        }
    }
